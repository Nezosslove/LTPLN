...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Predict with ltpln in miniset"
    )

    parser.add_argument(
        "--path_model",
        type=str,
        default=os.path.join(here,"model","ltpln_default"),
        help="model path",
    )
    parser.add_argument(
        "--output_file",
        type=str,
        default=os.path.join(here,"results","ltpln_res"),
    ...
    pre = ltpln.predict(model,x_test,args.positive_index)
    if positive_index == 0:
        pre = pre[:,0]
    else:
        pre = pre[:,1]
